...
from game import NO_GAME, CHALLENGERS
...
@route('/challengers')
def routeChallengersHome():
    return load(pageChallengersHome(), CHALLENGERS)

# Challengers page : home

def pageChallengersHome():
    lines = [
        '<h2>Choisissez vos options :</h2>',
        '<form>',
            '<input type="checkbox" name="options" value="base" onchange="loadFiveSets()"> Challengers!<br>',
            '<input type="checkbox" name="options" value="beach" onchange="loadFiveSets()"> Challengers! Beach Cup<br>',
            '<input type="checkbox" name="options" value="rumble" onchange="loadFiveSets()"> Challengers! Rumble<br>',
        '</form>',
        '<p id="result">Aucune option sélectionnée.</p>',
    ]
    return "".join(lines)
...
coreDarkTitle = "WIP: " + coreTitle + " + " + darkTitle
coreDarkMasterminds = coreMasterminds + darkMasterminds
coreDarkSchemes = coreSchemes + darkSchemes
coreDarkVillains = coreVillains + darkVillains
coreDarkHenchmen = coreHenchmen + darkHenchmen
coreDarkHeroes = coreHeroes + darkHeroes

coreDarkRokTitle = "WIP: " + coreTitle + " + " + darkTitle + " + " + rokTitle
coreDarkRokMasterminds = coreMasterminds + darkMasterminds + rokMasterminds
coreDarkRokSchemes = coreSchemes + darkSchemes + rokSchemes
coreDarkRokVillains = coreVillains + darkVillains + rokVillains
coreDarkRokHenchmen = coreHenchmen + darkHenchmen + rokHenchmen
coreDarkRokHeroes = coreHeroes + darkHeroes + rokHeroes

# Legendary routes

@route('/legendary/<nbPlayers>')
def routeLegendaryHome(nbPlayers):
    return load(pageLegendaryHome(nbPlayers))

### Legendary: Pick mastermind and scheme pages

@route('/legendary/core/mastermind-and-scheme/<nbPlayers>')
def routeLegendaryCoreMastermindAndScheme(nbPlayers):
    return load(pageLegendaryPickMastermindAndScheme(nbPlayers, coreTitle, coreMasterminds, coreSchemes, 'core'))

@route('/legendary/core-dark/mastermind-and-scheme/<nbPlayers>')
def routeLegendaryCoreDarkMastermindAndScheme(nbPlayers):
    return load(pageLegendaryPickMastermindAndScheme(nbPlayers, coreDarkTitle, coreDarkMasterminds, coreDarkSchemes, 'core-dark'))

@route('/legendary/core-dark-rok/mastermind-and-scheme/<nbPlayers>')
def routeLegendaryCoreDarkRokMastermindAndScheme(nbPlayers):
    return load(pageLegendaryPickMastermindAndScheme(nbPlayers, coreDarkRokTitle, coreDarkRokMasterminds, coreDarkRokSchemes, 'core-dark-rok'))

@route('/legendary/core-rok/mastermind-and-scheme/<nbPlayers>')
def routeLegendaryCoreRokMastermindAndScheme(nbPlayers):
    return load(pageLegendaryPickMastermindAndScheme(nbPlayers, coreRokTitle, coreRokMasterminds, coreRokSchemes, 'core-rok'))

### Legendary: Pick all pages
...
def routeLegendaryCoreAll(nbPlayers, mastermindIndex, schemeIndex):
    return load(pageLegendaryPickAll(nbPlayers, coreMasterminds[int(mastermindIndex)], coreSchemes[int(schemeIndex)], coreTitle, coreVillains, coreHenchmen, coreHeroes))

@route('/legendary/core-dark/all/<nbPlayers>/<mastermindIndex>/<schemeIndex>')
def routeLegendaryCoreDarkAll(nbPlayers, mastermindIndex, schemeIndex):
    return load(pageLegendaryPickAll(nbPlayers, coreDarkMasterminds[int(mastermindIndex)], coreDarkSchemes[int(schemeIndex)], coreDarkTitle, coreDarkVillains, coreDarkHenchmen, coreDarkHeroes))

@route('/legendary/core-dark-rok/all/<nbPlayers>/<mastermindIndex>/<schemeIndex>')
def routeLegendaryCoreDarkRokAll(nbPlayers, mastermindIndex, schemeIndex):
    return load(pageLegendaryPickAll(nbPlayers, coreDarkRokMasterminds[int(mastermindIndex)], coreDarkRokSchemes[int(schemeIndex)], coreDarkRokTitle, coreDarkRokVillains, coreDarkRokHenchmen, coreDarkRokHeroes))
...
def routeLegendaryCoreRokAll(nbPlayers, mastermindIndex, schemeIndex):
    return load(pageLegendaryPickAll(nbPlayers, coreRokMasterminds[int(mastermindIndex)], coreRokSchemes[int(schemeIndex)], coreRokTitle, coreRokVillains, coreRokHenchmen, coreRokHeroes))
...
def pageLegendaryHome(nbPlayers):
    lines = [
        ...
        '<table border="0" cellpadding="20">',
            ...
            '<tr>',
                '<td><font face="Verdana" size="10">',
                    '<a href="/legendary/core-dark/mastermind-and-scheme/' + nbPlayers + '">',
                        coreDarkTitle,
                    '</a>',
                '</td>',
            '</tr>',
            '<tr>',
                '<td><font face="Verdana" size="10">',
                    '<a href="/legendary/core-dark-rok/mastermind-and-scheme/' + nbPlayers + '">',
                        coreDarkRokTitle,
                    '</a>',
                '</td>',
            '</tr>',
...
def pageLegendaryPickMastermindAndScheme(nbPlayers, title, masterminds, schemes, subpath):
    ...
    lines = [
        ...
        '<div style="position: absolute; bottom: 20px;">',
            '&emsp;&emsp;Rafraîchir pour changer le tirage',
        '</div>',
        ]
    return "".join(lines)
